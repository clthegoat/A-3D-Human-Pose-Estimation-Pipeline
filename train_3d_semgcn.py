...
def validation(model, val_loader,p3d_mean, p3d_std):
    ...
    with torch.no_grad():
        ...
        for _, (_, pose3d, _, pose2d, _) in enumerate(val_loader):
            p2d = pose2d.to(DEVICE).float().view([-1, 17, 2])
...
def train(model, train_loader, val_loader, optimizer, p3d_mean, p3d_std, traincounter):
    ...
    for _, (_, pose3d, pose3d_norm, pose2d, _) in enumerate(loader):
        p3d_gt_norm, p2d = pose3d_norm.to(DEVICE), pose2d.to(DEVICE).float().view([-1, 17, 2])
...
def main():
    ...
    # load model
    model = SemGCN(adj=config.hr_gcn.ADJ,
    ...
    while epoch < 20:
        ...
        torch.save(model.state_dict(), MODEL_PATH + str(epoch)+SEMGCN)
