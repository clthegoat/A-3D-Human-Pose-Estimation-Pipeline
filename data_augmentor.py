#!/usr/bin/env python
# This code is based on https://github.com/isarandi/synthetic-occlusion.git
...
def main():
    # set path to Pascal VOC data
    voc_dataset_path = VOC_DATASET

    print('Loading occluders from Pascal VOC dataset...')
    occluders = load_occluders(pascal_voc_root_path=voc_dataset_path)
    print('Found {} suitable objects'.format(len(occluders)))
    img_list = sorted(os.listdir(FILE_H36_IMG_ori))
    ...
    for item in train_list:
        ...
        np.random.seed()
        p = np.random.rand(1)
        img = imread(FILE_H36_IMG_ori + item + '.jpg')
        original_im = cv2.resize(img, (256, 256))
        if p >= 0.5:
            occluded_im = occlude_with_objects(original_im, occluders)
            image = Image.fromarray(occluded_im, 'RGB')
        else:
            image = Image.fromarray(original_im, 'RGB')
        image.save(FILE_H36_IMG_aug + item + '.jpg')
